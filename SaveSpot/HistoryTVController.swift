...
class HistoryTVController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as! HistoryTVCell
        // Configure the cell...
        let spot = spotList[(indexPath as NSIndexPath).row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowDetail" {
            let locationDetailVC = segue.destination as! ShowLocationVC
            
            // Get the cell that generated this segue.
            if let selectedSpotCell = sender as? HistoryTVCell {
                let indexPath = tableView.indexPath(for: selectedSpotCell)!
                let selectedSpot = spotList[(indexPath as NSIndexPath).row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            spotList.remove(at: (indexPath as NSIndexPath).row)
